...
class MusanImporter:
    def __init__(self, config):
        self.source_path = config['datasets']['Musan']['source_path']
        self.target_dir = os.path.join(config['target_dir'], 'Musan')
        ...
        for split in ['Test', 'Dev', 'Train']:
            if not os.path.exists(os.path.join(self.csv_path, f'{split}_musan.csv')):
                with open(os.path.join(self.csv_path, f'{split}_musan.csv'), 'w') as file:
                    header = csv.writer(file)
                    header.writerow(['index', 'path', 'label'])

    def import_dataset(self):
        print(f"{'-' * 70}| Processing Noise (Musan) Data |{'-' * 70}")

        arr = np.array([])
        for audio in glob(os.path.join(self.source_path, 'noise', '*', '*.wav')):
    ...
    def process_data(self, audios, split):
        ...
        for file_name in tqdm(audios):
            ...
            labels_path = os.path.join(self.csv_path, f'{split}_musan.csv')

            with open(labels_path, 'a+') as file:
                csvwriter = csv.writer(file)
                csvwriter.writerow([index, new_audio_filepath, 'Other'])
